...
const ManagerFiles = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [recipientsRes, receivedRes, sentRes, allRes] = await Promise.all([
          filesAPI.getRecipients(),
          filesAPI.getReceivedFiles(),
          filesAPI.getSentFiles(),
          filesAPI.getUserFiles()
        ]);
        
        setTeamMembers(recipientsRes.data.data || []);
        setReceivedFiles(receivedRes.data.data || []);
        setSentFiles(sentRes.data.data || []);
        setTeamFiles(allRes.data.data.files || []);
      } catch(e) { 
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {(activeTab === 'team' ? teamFiles : activeTab === 'received' ? receivedFiles : sentFiles)
            ...
            .map((file, i) => {
              ...
              const isUnread = activeTab === 'received' && !file.readBy?.some((r: any) => r.user === currentUserId);
                ...
                <motion.div key={file._id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="bg-card border border-border rounded-2xl p-4 flex items-center gap-4 hover:border-foreground/20 transition-all shadow-sm">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-2 gap-y-1 text-[10px] text-muted-foreground mt-1">
                      ...
                      {activeTab === 'team' && file.uploadedBy && (
                        <span>• By {file.uploadedBy.fullName}</span>
                      )}
                      
                      {activeTab === 'received' && file.uploadedBy && (
                        <span className="flex items-center gap-1">
                          • From: <span className="font-medium">{file.uploadedBy.fullName}</span> 
                          <span className="bg-muted px-1.5 py-0.5 rounded-full text-[8px] capitalize leading-none">{file.uploadedBy.role?.replace('_', ' ')}</span>
                        </span>
                      )}
                      
                      {activeTab === 'sent' && file.sharedWith && (
                        <span className="flex items-center gap-1 flex-wrap">
                          • To: {file.sharedWith.map((r: any) => (
                            <span key={r._id} className="bg-muted px-1.5 py-0.5 rounded-full text-[8px] capitalize leading-none">{r.fullName}</span>
                          ))}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    {activeTab === 'sent' && (
                       <span className="text-[10px] text-muted-foreground bg-muted/50 px-2 py-1 rounded-full whitespace-nowrap">
                         {file.readBy?.length || 0}/{file.sharedWith?.length || 0} Read
                       </span>
                    )}
                    <button onClick={handleDownload} className="text-xs font-bold px-3 py-1.5 rounded-full bg-muted hover:bg-muted-foreground/10 transition-colors whitespace-nowrap">
                      {activeTab === 'received' ? 'View' : 'Download'}
                    </button>
                  </div>
                </motion.div>
              );
