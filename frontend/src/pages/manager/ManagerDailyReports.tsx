...
const ManagerDailyReports = () => {
  ...
  const handleToggleShare = async (reportId: string) => {
    try {
      const res = await managerAPI.toggleReportSharing(reportId);
      setReports(reports.map(r => r._id === reportId ? { ...r, sharedWithClient: res.data.data.sharedWithClient } : r));
      if (selectedReport?._id === reportId) {
        setSelectedReport({ ...selectedReport, sharedWithClient: res.data.data.sharedWithClient });
      }
      showToast(res.data.data.sharedWithClient ? 'Shared with Client' : 'Unshared with Client', 'success');
    } catch (e) {
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 border-b border-border pb-10 mb-10">
                  <div className="space-y-4">
                    <div className="flex items-center flex-wrap gap-3">
                       ...
                       <button 
                          onClick={() => handleToggleShare(selectedReport._id)}
                          className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all flex items-center gap-2 ${
                            selectedReport.sharedWithClient ? 'bg-green-50 text-green-600 border-green-200 shadow-sm' : 'bg-muted text-muted-foreground border-border'
                          }`}
                        >
                          <Send className="w-3 h-3" /> {selectedReport.sharedWithClient ? 'Shared with Client' : 'Share with Client'}
                        </button>
                          ...
                          <div className="flex flex-wrap gap-2">
                             {selectedReport.attachments?.length > 0 ? (
                                selectedReport.attachments.map((file: any, i: number) => {
                                  const isAbsolute = file.url?.startsWith('http') || file.url?.startsWith('data:') || file.url?.startsWith('blob:');
                                  const url = isAbsolute ? file.url : `${import.meta.env.VITE_API_URL?.replace('/api', '') || (import.meta.env.DEV ? 'http://localhost:5000' : '')}${file.url?.startsWith('/') ? '' : '/'}${file.url}`;
                                  return (
                                    <a 
                                      key={i}
                                      href={url} 
                                ...
                                <a 
                                  href={(selectedReport.attachmentUrl?.startsWith('http') || selectedReport.attachmentUrl?.startsWith('data:') || selectedReport.attachmentUrl?.startsWith('blob:')) ? selectedReport.attachmentUrl : `${import.meta.env.VITE_API_URL?.replace('/api', '') || (import.meta.env.DEV ? 'http://localhost:5000' : '')}${selectedReport.attachmentUrl?.startsWith('/') ? '' : '/'}${selectedReport.attachmentUrl}`} 
                                  target="_blank" 
                                  rel="noopener noreferrer"
                                  className="px-4 py-1.5 rounded-full bg-blue-500 text-white text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:scale-105 transition-all text-center leading-none"
                                >
