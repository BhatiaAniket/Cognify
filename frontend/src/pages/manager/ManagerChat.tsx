  ...
  useEffect(() => {
    ...
    const fetchContacts = async () => {
      try {
        ...
        // Deduplicate by _id on the frontend as a safety net
          ...
          {activeTab === 'contacts' ? (
            ...
            loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-xs text-muted-foreground mt-1">Go to Team tab to start a chat</p>
              </div>
            ) : (
            ...
            <div className="p-4 border-t border-border bg-card">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-3 rounded-xl border border-border bg-background text-sm focus:outline-none focus:border-foreground shadow-sm"
                />
                <motion.button
                  whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  onClick={sendMessage}
                  disabled={!newMessage.trim()}
                  className="p-3 rounded-xl bg-foreground text-background disabled:opacity-40 shadow-sm"
                >
