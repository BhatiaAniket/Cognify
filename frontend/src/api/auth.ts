...
const API_URL = import.meta.env.VITE_API_URL ? `${import.meta.env.VITE_API_URL}/auth` : (import.meta.env.DEV ? 'http://localhost:5000/api/auth' : '/api/auth');
...
export const authAPI = {
  ...
  resetPassword: (token: string, newPassword: string) => api.post(`/reset-password/${token}`, { newPassword }),
};
