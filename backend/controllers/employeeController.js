...
const { generateEmployeeAIReport } = require('../services/ai.service');
...
exports.getMeetings = async (req, res) => {
  try {
    ...
    const allMeetings = await Meeting.find({
      ...
      .populate('createdBy participants', 'fullName email role avatar')
      .sort({ startTime: 1 });
...
exports.requestMeeting = async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: 'Meeting request sent', data: request });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Server error', errors: [] });
  }
};
