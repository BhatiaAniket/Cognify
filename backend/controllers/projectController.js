...
exports.assignClient = async (req, res) => {
  try {
    ...
    // 1. Find the project and verify it belongs to this company
    const project = await Project.findOne({ _id: req.params.id, company: req.companyId });
    if (!project) {
      return res.status(404).json({ success: false, message: 'Project not found' });
    }

    // 2. Verify client belongs to this company and exists
    const client = await User.findOne({ _id: clientId, company: req.companyId, role: 'client' });
    if (!client) {
      return res.status(404).json({ success: false, message: 'Client not found in this company' });
    }

    // 3. Update Client (User) document
    client.assignedProject = project._id;
    client.assignedManager = project.manager;
    ...
    // 5. Populate updated client with project and manager and return it
    const updatedClient = await User.findById(clientId)
      .populate('assignedProject', 'name description status progress deadline')
    ...
    await ActivityLog.create({
      company: req.companyId,
      user: req.user._id,
      action: `Assigned client ${client.fullName} to project "${project.name}"`,
      entity: 'project',
      entityId: project._id,
    });
