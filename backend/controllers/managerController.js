...
exports.getClients = async (req, res) => {
  try {
    ...
    // Find all projects assigned to this manager and get their clients
    const projects = await Project.find({ manager: managerId, company: companyId })
      .populate('client', 'fullName email lastActive avatar');
...
// ── DEMANDS ──────────────────────────────────────────────────────────────────
exports.getClientDemands = async (req, res) => {
  try {
    const managerId = toObjectId(req.user._id);
    const { clientId, projectId } = req.query;

    const query = { manager: managerId };
    if (clientId) query.client = toObjectId(clientId);
    if (projectId) query.project = toObjectId(projectId);

    const demands = await require('../models/Demand').find(query)
...
exports.updateDemandStatus = async (req, res) => {
  try {
    ...
    const demand = await require('../models/Demand').findOneAndUpdate(
      { _id: req.params.id, manager: managerId },
      { status, managerComment: managerComment || '' },
      { new: true }
    );
    ...
    await Notification.create({
      ...
      message: `Your demand "${demand.title}" status has been updated to: ${status}`,
      type: 'demand_updated',
    ...
    try {
      const { emitToUser } = require('../socket');
      emitToUser(demand.client.toString(), 'notification', { message: 'Demand status updated' });
    } catch (e) {
      console.error('Socket emit error:', e.message);
    }
...
// ── SHARING ──────────────────────────────────────────────────────────────────
exports.toggleReportSharing = async (req, res) => {
  try {
    const managerId = toObjectId(req.user._id);
    const report = await DailyReport.findOne({ _id: req.params.id, manager: managerId });
    if (!report) return res.status(404).json({ success: false, message: 'Report not found' });

    report.sharedWithClient = !report.sharedWithClient;
    await report.save();

    return res.json({ success: true, data: { sharedWithClient: report.sharedWithClient } });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
};

exports.toggleMeetingSharing = async (req, res) => {
  try {
    const managerId = toObjectId(req.user._id);
    const Meeting = require('../models/Meeting');
    const meeting = await Meeting.findOne({ _id: req.params.id, createdBy: managerId });
    if (!meeting) return res.status(404).json({ success: false, message: 'Meeting not found' });

    meeting.sharedWithClient = !meeting.sharedWithClient;
    await meeting.save();

    return res.json({ success: true, data: { sharedWithClient: meeting.sharedWithClient } });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.getMeetingRequests = async (req, res) => {
  try {
    ...
    const requests = await MeetingRequest.find({
      companyId,
      requestedTo: managerId,
      status: 'pending'
    }).populate('requestedBy', 'fullName email role avatar').sort({ createdAt: -1 });
...
exports.handleMeetingRequest = async (req, res) => {
  try {
    const managerId = toObjectId(req.user._id);
    const { notificationId, action } = req.body; // Actually requestId passed as notificationId in frontend
    const MeetingRequest = require('../models/MeetingRequest');
    const Notification = require('../models/Notification');
    
    // We expect `action` to be 'accept' or 'decline'
    ...
    if (action === 'accept') {
      ...
      const meeting = await Meeting.create({
        ...
        createdBy: managerId,
        participants: [request.requestedBy._id, managerId],
        status: 'scheduled'
      });
...
exports.shareMeetingSummary = async (req, res) => {
  try {
    const meeting = await require('../models/Meeting').findById(req.params.id);
    if (!meeting) return res.status(404).json({ success: false, message: 'Meeting not found' });
    if (!meeting.summary) {
      return res.status(400).json({ success: false, message: 'Generate summary first' });
    }
    ...
    for (const clientId of clientIds) {
      await SharedSummary.findOneAndUpdate(
        { meetingId: meeting._id, sharedWith: clientId },
        {
      ...
      try {
        emitToUser(clientId.toString(), 'summary:shared', {
    ...
    return res.json({ success: true, message: 'Summaries shared successfully' });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
};
