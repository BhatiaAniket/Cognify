...
const initCronJobs = () => {
  ...
  // Check for stale client meeting requests every 4 hours
  cron.schedule('0 */4 * * *', async () => {
    await checkStaleMeetingRequests();
  });
};
...
/**
 * Check for client meeting requests pending > 48 hours and notify admin
 */
const checkStaleMeetingRequests = async () => {
  try {
    const MeetingRequest = require('../models/MeetingRequest');
    const fortyEightHoursAgo = new Date(Date.now() - 48 * 60 * 60 * 1000);

    const staleRequests = await MeetingRequest.find({
      requesterRole: 'client',
      status: 'pending',
      createdAt: { $lte: fortyEightHoursAgo },
    }).populate('requestedBy', 'fullName').populate('requestedTo', 'fullName');

    for (const request of staleRequests) {
      // Find company admins
      const admins = await User.find({ company: request.companyId, role: 'company_admin' });
      for (const admin of admins) {
        ...
        await Notification.create({
          ...
          user: admin._id,
          type: 'general',
          title: 'Pending Client Meeting Request',
          message: `Meeting request from client ${request.requestedBy?.fullName || 'N/A'} has been pending for over 48 hours (Assigned Manager: ${request.requestedTo?.fullName || 'N/A'}).`,
    ...
    console.error('Stale meeting request check error:', error.message);
  }
};
